package com.main.data_show.consts;

public class ApplicationConsts {
    public static String SYS_DEMO_IMPORT_DATA_BASE_PATH = "sys.demo.import.data.base.path";

    public static String SYS_DEMO_INSTANT_EXPORT_DATA_BASE_PATH = "sys.demo.instant.export.date.base.path";

    public static String SYS_DEMO_INSTANT_EXPORT_IMG_BASE_PATH = "sys.demo.instant.export.img.base.path";

    public static String SYS_DEMO_USAGE_EXPORT_DATA_BASE_PATH = "sys.demo.usage.export.date.base.path";

    public static String SYS_DEMO_USAGE_EXPORT_IMG_BASE_PATH = "sys.demo.usage.export.img.base.path";

    public static String SYS_DEMO_EXPORT_IMG_WIDTH = "sys.demo.export.img.width";

    public static String SYS_DEMO_EXPORT_IMG_HEIGHT = "sys.demo.export.img.height";

    public static String SYS_DEMO_IMPORT_DATA_REMARK_BASE_PATH = "sys.demo.import.date.remark.base.path";

    public static String SYS_POINT_USAGE_RECORD_DEF_HOUR = "sys.point.usage_record_def_hour";

    public static String SYS_POINT_USAGE_RECORD_DEF_WEEK_NUM = "sys.point.usage_record_def_week_num";
}
